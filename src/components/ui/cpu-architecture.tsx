...
/**
 * CPU architecture animated SVG.
 *
 * 8 curved paths flow into a central CPU rectangle, each carrying a colored
 * "light particle" along the path via CSS `offset-path`. Used in Vguard as
 * the visual metaphor for "many signal streams → one verdict (Vibe Score)".
 *
 * Pure SVG + CSS; no external deps beyond `cn()`. Sizes via `width`/`height`
 * props (defaults to 100%/100%) so a parent wrapper controls the actual
 * pixel dimensions on mobile vs desktop.
 *
 * Per-line offset-path animations live in `src/index.css` under the
 * `.cpu-architecture` / `.cpu-line-N` selectors.
 */
...
export function CpuArchitecture({
  ...
  return (
    <svg
      ...
      viewBox="0 0 200 100"
      role="img"
      aria-label="CPU architecture diagram with animated signal streams"
    >
      {/* Paths */}
      <g
        stroke="currentColor"
      ...
      <g>
        ...
        <text
          // Auto-center inside the 30×20 CPU box (centered on x=100, y=50) so
          // the label looks balanced regardless of length — "secure" / "SCORE"
          // / a 4-letter word all sit visually centered.
          x="100"
          y="50"
          fontSize="6.5"
          textAnchor="middle"
